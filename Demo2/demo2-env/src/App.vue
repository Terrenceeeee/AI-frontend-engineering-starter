    ...
    <table border="1" cellpadding="12" style="border-collapse: collapse; width: 100%; font-size: 14px;">
      ...
      <tbody>
        <tr>
          ...
          <td><strong style="color: #42b883;">{{ env.MODE }}</strong></td>
        </tr>
        <tr>
          <td><code>import.meta.env.COMMAND</code></td>
          <td><strong style="color: #42b883;">{{ env.COMMAND }}</strong></td>
        </tr>
        <tr>
          <td><code>import.meta.env.DEV</code></td>
          <td><strong style="color: #42b883;">{{ String(env.DEV) }}</strong></td>
        </tr>
        <tr>
          <td><code>import.meta.env.PROD</code></td>
          <td><strong style="color: #42b883;">{{ String(env.PROD) }}</strong></td>
        </tr>
        <tr>
          <td><code>import.meta.env.VITE_API_BASE_URL</code></td>
          <td><strong style="color: #42b883;">{{ env.VITE_API_BASE_URL }}</strong></td>
        </tr>
        <tr>
          <td><code>import.meta.env.VITE_ENABLE_DEBUG</code></td>
          <td><strong style="color: #42b883;">{{ String(env.VITE_ENABLE_DEBUG) }}</strong></td>
        </tr>
        <tr>
          <td><code>import.meta.env.VITE_APP_TITLE</code></td>
          <td><strong style="color: #42b883;">{{ env.VITE_APP_TITLE }}</strong></td>
        </tr>
        <tr style="background: #fff3cd;">
          <td><code style="background: #fff3cd;">import.meta.env.DATABASE_PASSWORD</code></td>
          <td><strong style="color: red;">{{ String(env.DATABASE_PASSWORD) }}</strong></td>
        </tr>
      </tbody>
    </table>

    <div
      style="margin-top: 24px; padding: 16px 20px; background: #f0f9ff; border-left: 4px solid #42b883; border-radius: 4px;">
      <p style="margin: 0; font-weight: bold;">📌 验证要点：</p>
      <ul style="margin-top: 8px;">
        <li>只有 <code>VITE_</code> 开头的变量会暴露给前端（安全隔离）</li>
        <li><code>DATABASE_PASSWORD</code> 应为 <span style="color: red; font-weight: bold;">undefined</span>（红色显示）</li>
        <li><code>MODE</code> 会随启动命令变化</li>
        <li><code>COMMAND</code> 表示当前执行的是 <code>serve</code>（开发）还是 <code>build</code>（构建）</li>
      </ul>
    </div>

    <div style="margin-top: 16px; padding: 16px 20px; background: #f5f5f5; border-radius: 4px;">
      <p style="margin: 0; font-size: 13px; color: #666;">
        ⚡ 当前运行在：<strong>{{ env.MODE }}</strong> 模式
        （<code>pnpm {{ env.COMMAND === 'serve' ? 'dev' : 'build' }}</code>）
      </p>
    </div>
  </div>
</template>
